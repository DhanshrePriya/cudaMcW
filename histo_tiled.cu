...
__global__ void kernel(int *d_image, int *d_hist, int size) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    __shared__ int img[TILE];  
    __shared__ int hist[NUM_BINS];  
    hist[threadIdx.x] = 0; 
    img[threadIdx.x] = d_image[idx]; 
    if (idx < SIZE) atomicAdd(&hist[img[threadIdx.x]], 1); 
    __syncthreads();
    atomicAdd(&d_hist[threadIdx.x],  hist[threadIdx.x]);
}

// CPU Histogram 
void cpuHistogram(int *h_image, int *h_hist, int size) {
...
int main() {
    ...
    size_t imgBytes = imgSize * sizeof(int);
    size_t histBytes = NUM_BINS * sizeof(int);

    // img and hist in cpu 
    int *h_image = new int[imgSize];
